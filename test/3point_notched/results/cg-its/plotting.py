...
p = re.compile("A0 converged in: ([0-9]*)")
...
for i in range(len(filenames)):
    ...
    plt.plot(its0, linestyle="none", marker=markers[i], label=labels[i])

plt.legend()
plt.xlabel("Load step")
plt.ylabel("Avg. number of Krylov iterations")
plt.tight_layout()
plt.savefig("malvar-krylov-its.pdf")
